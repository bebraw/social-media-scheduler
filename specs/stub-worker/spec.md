...
- **Entry points:** `wrangler dev` via `src/worker.ts`, authenticated `GET /`, `GET /compose`, and `GET /history` page routes, development-only `GET /demo` and `POST /demo/queue`, `npm run account:create` for auth user management, and the Worker scheduled handler for automated backups
- **Source layout:** `src/worker.ts` routes requests, `src/auth/` holds auth primitives and D1-backed auth state helpers, `src/backup/` holds the backup export flow, `src/demo/` holds development-only demo gating plus seeded/local demo state, `src/history/` holds sent-post history loading for normal routes, `src/queue/` holds channel constraint logic, `src/api/` holds API handlers, and `src/views/` holds the queue, compose, history, and demo HTML renderers plus the shared interaction script.
...
- When: they open `/`
- Then: they see the default queue view with queue metrics, queued-post review space, and links to the dedicated composer without seeded demo data
