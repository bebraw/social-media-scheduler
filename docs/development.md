...
After sign-in, the default authenticated surface is now the `Queue` view at `/`, which includes the per-channel posting schedule editor for whichever providers are currently configured in Settings. The dedicated `Compose` and `History` views also derive their visible accounts and filters from those saved channel connections instead of assuming one fixed card per provider. Settings now supports adding, rotating, and revoking per-account provider connections without manual D1 edits.

Posting schedules are stored in D1 `app_state` as Cloudflare-cron-compatible expressions derived from the queue UI's weekday and UTC time controls. The deployed Worker now runs on a fixed 15-minute publishing poll plus a separate daily backup cron, and the scheduled runtime evaluates the saved channel schedules directly instead of requiring operators to copy every schedule change back into `wrangler.jsonc`.
